module Dalmatian.Editor.Token exposing (TokenValue, findByPosition, findStringByPosition, find, delete, update, upsert, getNextRank)
...
update tokens tokenId token =
    tokens |> List.map (\t -> if t.uid == tokenId then { t | value = token } else t)
